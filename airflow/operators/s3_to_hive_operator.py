...
class S3ToHiveTransfer(BaseOperator):
    ...
    def execute(self, context):
        ...
        with TemporaryDirectory(prefix='tmps32hive_') as tmp_dir,\
            ...
            if not self.headers:
                _log.info("Loading file {0} into Hive".format(f.name))
                self.hive.load_file(
                    f.name,
                    self.hive_table,
                ...
                if self.input_compressed:
                    _log.info("Uncompressing file {0}".format(f.name))
                    fn_uncompressed = uncompress_file(f.name,
                                                      file_ext,
                                                      tmp_dir)
                    _log.info("Uncompressed to {0}".format(fn_uncompressed))
                ...
                if self.check_headers:
                    _log.info("Matching file header against field_dict")
                ...
                _log.info("Removing header from file {0}".
                             format(fn_uncompressed))
                headless_file = (
                    self._delete_top_row_and_compress(fn_uncompressed,
                                                      file_ext,
                                                      tmp_dir))
                _log.info("Headless file {0}".format(headless_file))
                _log.info("Loading file {0} into Hive".format(headless_file))
    ...
    def _match_headers(self, header_list):
        ...
        if len(field_names) != len(header_list):
            _log.warning("Headers count mismatch"
                         "File headers:\n {header_list}\n"
                         "Field names: \n {field_names}\n"
                         "".format(**locals()))
            return False
